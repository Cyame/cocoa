...
def resolve_api_key(api_key_ref: str) -> str:
    """Resolve secret from env by ref name. Never store plaintext keys in DB."""
    key = os.environ.get(api_key_ref, "")
    if not key:
        raise LLMError(
            "errors.llm.api_key_missing",
            f"Environment variable {api_key_ref!r} is not set",
        )
    return key
...
async def fetch_custom_models(
    ...
    """Return (items, error). Never falls back to models.dev."""
    import httpx
    ...
    try:
        async with httpx.AsyncClient(
            verify=bool(provider.verify_ssl), timeout=15.0
        ) as client:
