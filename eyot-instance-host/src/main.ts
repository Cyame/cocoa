...
async function main(): Promise<void> {
  const apiUrl = process.env.EYOT_API_URL?.trim() || "http://127.0.0.1:4510";
  const instanceId = requireEnv("EYOT_INSTANCE_ID");
  const proxyToken = requireEnv("EYOT_PROXY_TOKEN");
  const tunnelOverride = process.env.EYOT_TUNNEL_URL?.trim();
  const workspace = process.env.EYOT_WORKSPACE_PATH?.trim() || "/data";
  const healthPort = Number(process.env.EYOT_HOST_PORT || "8080");
