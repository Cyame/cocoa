...
- **Local mode** (``EYOT_POD_MODE != "true"``): in-process event emit +
  register HARNESS_CONTROL_SENT handler (P8 plan).

- **K8s mode** (``EYOT_POD_MODE == "true"``): HTTP POST to backend's
...
def is_k8s_pod_mode() -> bool:
    ...
    return os.environ.get("EYOT_POD_MODE", "").lower() == "true"


def get_api_url() -> str:
    """Backend HTTP base URL, e.g. ``http://eyot-backend-svc:4510``."""
    return os.environ.get("EYOT_API_URL", "http://eyot-backend-svc:4510")


def get_api_token() -> str:
    """Backend internal endpoint token (matches ``EYOT_API_TOKEN`` env on backend)."""
    return os.environ.get("EYOT_API_TOKEN", "")


def get_proxy_token() -> str:
    """Per-instance proxy token (matches ``Instance.proxy_token`` in DB)."""
    return os.environ.get("EYOT_PROXY_TOKEN", "")


def get_instance_id() -> str:
    """Own instance ID (matches ``Instance.id`` in DB)."""
    return os.environ.get("EYOT_INSTANCE_ID", "")
