...
LABEL_INSTANCE_ID = "eyot/instance-id"
LABEL_IMAGE_TAG = "eyot/image-tag"
...
def build_labels(instance_id: str, image_tag: str = "") -> dict[str, str]:
    ...
    * ``app.kubernetes.io/managed-by`` — always ``"eyot"``
    * ``eyot/instance-id`` — links the resource back to a Eyot instance

    Optional:

    * ``eyot/image-tag`` — included only when ``image_tag`` is non-empty
    """
    labels: dict[str, str] = {
...
def build_resource_quota(
    ...
    ``name`` defaults to ``"eyot-quota"`` when omitted so callers can
    reference the same quota across multiple calls.
    """
    return V1ResourceQuota(
        metadata=V1ObjectMeta(name=name or "eyot-quota", namespace=ns),
...
def build_deployment(
    ...
    * ``env_vars`` — additional literal env vars (``FOO=bar``)
    ...
    container = V1Container(
        name=name[:63],
        image=image,
        image_pull_policy=os.environ.get("EYOT_INSTANCE_IMAGE_PULL_POLICY", "IfNotPresent"),
