"""P11c Todo 10: ``EYOT_PROXY_TOKEN`` reaches the K8s deployment env and
...
1. ``test_proxy_token_in_deployment_env`` — :func:`deploy_service.deploy_instance`
   places ``proxy_token`` into ``ctx.env_vars["EYOT_PROXY_TOKEN"]`` so the
...
# ── 1. EYOT_PROXY_TOKEN injected into the K8s env ────────────────────────
...
async def test_proxy_token_in_deployment_env(
    ...
    assert ctx.env_vars["EYOT_PROXY_TOKEN"] == proxy_token
    assert ctx.env_vars["EYOT_PROXY_TOKEN"] != "", (
        "EYOT_PROXY_TOKEN must not be empty after injection"
    )
