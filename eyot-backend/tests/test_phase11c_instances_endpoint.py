...
2. ``test_deploy_local_mode_falls_back_to_db_transition`` — with
   ``EYOT_K8S_DISABLED=true`` (local dev), the deploy endpoint
...
3. ``test_delete_calls_k8s_delete_namespace`` — ``DELETE /instances/{id}``
   ...
   ``eyot-default-{workspace_path}`` namespace, with mocked clients.
...
async def test_deploy_calls_deploy_service(
    ...
    monkeypatch.delenv("EYOT_K8S_DISABLED", raising=False)
    monkeypatch.setenv("KUBECONFIG", "/tmp/eyot-fake-kubeconfig.yaml")
...
async def test_deploy_local_mode_falls_back_to_db_transition(
    ...
    """Local mode (EYOT_K8S_DISABLED=true) falls back to P7 DB transition.

    With no kubeconfig / service-account / GATEWAY_KUBECONFIG and
    ``EYOT_K8S_DISABLED=true`` set, ``_is_k8s_available()`` returns
    ...
    monkeypatch.setenv("EYOT_K8S_DISABLED", "true")
...
async def test_delete_calls_k8s_delete_namespace(
    ...
    ``eyot-inst-{id-compact-[:20]}`` convention and returns 204.
    """
    import app.core.db as db_mod
    ...
    expected_ns = f"eyot-inst-{instance.id.replace('-', '').lower()[:20]}"
