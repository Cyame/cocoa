...
async def test_cancel_deploy_cleans_namespace(
    ...
    assert namespace == f"eyot-inst-{ctx.instance_id.replace('-', '').lower()[:20]}"
...
async def test_internal_events_emit_writes_db(
    ...
    monkeypatch.setenv("EYOT_API_TOKEN", _TEST_TOKEN)
    body = {
...
async def test_proxy_token_env_injection(deploy_factory) -> None:
    ...
    assert ctx.env_vars["EYOT_PROXY_TOKEN"] == "secret-proxy"
    assert ctx.env_vars["EYOT_INSTANCE_ID"] == ctx.instance_id
    assert ctx.env_vars["EYOT_POD_MODE"] == "true"
    assert ctx.env_vars["EYOT_API_URL"]
    assert ctx.env_vars["EYOT_WORKSPACE_PATH"] == "/data"
