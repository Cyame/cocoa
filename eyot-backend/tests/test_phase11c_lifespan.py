...
1. ``test_lifespan_starts_event_watcher_in_k8s_mode`` — with
   ``EYOT_POD_MODE=true`` the lifespan calls
...
# ── 1. EYOT_POD_MODE=true → EventWatcher.start ────────────────────────────
...
async def test_lifespan_starts_event_watcher_in_k8s_mode(
    ...
    monkeypatch.setenv("EYOT_POD_MODE", "true")
...
async def test_lifespan_skips_event_watcher_in_local_mode(
    ...
    """``EYOT_POD_MODE`` unset ⇒ watcher.start is NOT called."""
    monkeypatch.delenv("EYOT_POD_MODE", raising=False)
    monkeypatch.setenv("EYOT_API_TOKEN", "preexisting-test-token")
    ...
    async with _swap_db(db_url):
        ...
        with TestClient(app) as _client:
            assert spy.start_calls == 0, (
                "start() must not be called when EYOT_POD_MODE is unset"
            )
            assert spy._task is None
