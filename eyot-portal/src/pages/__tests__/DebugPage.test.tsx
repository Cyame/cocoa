...
describe('DebugPage', () => {
  ...
  it('exports the current event list as a JSON download', async () => {
    const createObjectURLSpy = vi.fn().mockReturnValue('blob:eyot-events');
    ...
    expect(anchor?.download).toMatch(/^eyot-events-.*\.json$/);

    expect(revokeObjectURLSpy).toHaveBeenCalledWith('blob:eyot-events');
  });
