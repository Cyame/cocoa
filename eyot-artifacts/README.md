...
| Manifest | Purpose |
|----------|---------|
| `Dockerfile.instance-base` | Builds `eyot-instance-base:{engine-v}`: Node host + pinned pi CLI + vendored subagent extension (`~/.pi/agent/extensions/subagent/`). Agents dir kept empty — custom slugs fall back to this image + ConfigMap (v5.1 G9). |
| `Dockerfile.instance-ancestor` | Builds `eyot-instance-{slug}:{engine-v}` for the 5 始祖: `FROM base` + thin layer baking only the slug's enabled agent `.md` files into `~/.pi/agent/agents/`. Enabled set resolved by `scripts/build-instance-images.sh` from `builtin_presets.py` (`subagent_strategy.enabled`). |
...
| `service.yaml` | `ClusterIP` Service mapping port 8080. No external exposure — eyot-backend communicates with Instances from within the cluster. |
...
| `kustomization.yaml` | Groups all 5 YAML resources under a common `app: eyot-instance` label. Apply with `kubectl apply -k`. |
...
The v5.1 instance image family is `eyot-instance-base:{engine-v}` + one thin
layer per 始祖 (`eyot-instance-{fox|beaver|sparrow|coyote|lion}:{engine-v}`).
`engine-v` = pi runtime version (from `eyot-instance-host/package.json` pin);
enabled agent sets come from `eyot-backend/app/core/builtin_presets.py`.
...
EYOT_INSTANCE_REGISTRY=localhost:5000 bash scripts/build-instance-images.sh --push
```
...
#     eyot-artifacts/k8s/instance/*.yaml > /tmp/instance-rendered/
# Then apply:
